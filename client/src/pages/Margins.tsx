...
function buildWaterfallBars(
  ...
  return items.map((d, i) => {
    if (i === 0) {
      running = d.value;
      return { ...d, base: 0, display: d.value };
    }
...
export default function Margins() {
  ...
  const [sortField, setSortField] = useState<"margin" | "change" | "name">(
    "change"
  );
  ...
  const crisisBySector = useMemo(() => {
    if (!crisisData)
      return {} as Record<
        string,
        { severity: string; elementName: string; score: number }
      >;
    const map: Record<
      string,
      { severity: string; elementName: string; score: number }
    ...
    for (const [catId, sectorId] of Object.entries(sectorMap)) {
      ...
      const worst = cells.reduce((a, b) =>
        b.impactScore > a.impactScore ? b : a
      );
      const element = crisisData.elements.find(e => e.id === worst.elementId);
      map[catId] = {
        severity: worst.severity,
        elementName: element?.name ?? worst.elementId,
        score: worst.impactScore,
      };
    }
    return map;
  }, [crisisData]);
  ...
  const filteredSkus = useMemo(() => {
    ...
    return skus
      .filter(
        s =>
          filterCategory === "all" ||
          s.category.toLowerCase().replace(/[^a-z]/g, "-") === filterCategory
      )
      .sort((a, b) => {
      ...
      >
        ...
        >
          <div>
            ...
            >
              Live margin tracking — oil prices, freight rates &amp; disruption
              impact · Refreshes every 5 hours
            </p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {kpis && (
              ...
              >
                Brent ${kpis.brentPrice.toFixed(2)} · WTI $
                {kpis.wtiPrice.toFixed(2)} · Freight +
                {kpis.freightSurcharge.toFixed(1)}%
              </span>
            )}
            ...
            >
              <RefreshCw
                size={11}
                className={analysisFetching ? "animate-spin" : ""}
              />
              REFRESH
            </button>
          </div>
        ...
        >
          ...
          >
            ...
            <KpiCard
              ...
              value={
                kpis ? `-${Math.abs(kpis.oilPriceImpact).toFixed(1)}%` : "—"
              }
              sub={
                kpis
                  ? `Brent at $${kpis.brentPrice.toFixed(2)}/bbl`
                  : "Loading..."
              }
              color="#f97316"
              icon={TrendingDown}
              ...
              >
                <div>
                  <span className="panel-header">
                    MARGIN WATERFALL — COST EROSION BREAKDOWN
                  </span>
                  <p
                    style={{
                      fontFamily: "'Inter', sans-serif",
                ...
                <ResponsiveContainer width="100%" height={240}>
                  ...
                  >
                    ...
                    {analysis?.waterfall && (
                      <ReferenceLine
                        y={
                          analysis.waterfall.find(w => w.isCurrent)?.value ?? 25
                        }
                        stroke="#f97316"
                        strokeDasharray="4 4"
                        strokeWidth={1.5}
            ...
            <div className="ms-panel" style={{ padding: "16px" }}>
              ...
              >
                <div>
                  ...
                  >
                    Avg portfolio margin vs. target — derived from live oil
                    prices
                  </p>
                </div>
                {oilHistory && (
                  <LiveBadge lastUpdated={oilHistory.lastUpdated} />
                )}
              </div>
              <div
              ...
              >
                {[
                  { color: "#3b82f6", label: "Avg Margin" },
                  { color: "#f97316", label: "Target 30%", dashed: true },
                ].map(l => (
                  <div
                    key={l.label}
                    style={{
          ...
          <div className="ms-panel" style={{ padding: "16px" }}>
            ...
            >
              <div>
                ...
                >
                  Current margin vs. baseline — crisis impact signals from live
                  Hormuz data
                </p>
              </div>
              {!analysisLoading && (
                    ...
                    >
                      ...
                      {crisis && (
                        ...
                        >
                          <AlertTriangle
                            size={10}
                            style={{ color: crisisColor, flexShrink: 0 }}
                          />
                          <span
                            style={{
                              fontFamily: "'Inter', sans-serif",
