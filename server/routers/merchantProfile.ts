...
function shapeProfile(row: typeof merchantProfiles.$inferSelect) {
  return {
    ...
    sourcingRegions: row.sourcingRegions
      ? row.sourcingRegions
          .split(",")
          .map(s => s.trim())
          .filter(Boolean)
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db
        ...
        .set({
          ...
          ...(input.annualImportVolume !== undefined && {
            annualImportVolume: input.annualImportVolume,
          }),
          ...(input.sourcingRegions !== undefined && {
            sourcingRegions: input.sourcingRegions.join(", "),
          }),
          ...(input.productCategories !== undefined && {
            productCategories: input.productCategories.join(", "),
          }),
  ...
  addMarginSnapshot: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const sameMonth = existing.find(
        (r: typeof marginHistory.$inferSelect) => r.month === input.month
      );
      if (sameMonth) {
        // Update in place
        await db
          .update(marginHistory)
          .set({
